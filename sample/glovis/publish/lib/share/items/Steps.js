...
const Steps = ({type, contents, subContents, active, mode="normal", links, reverse=false, marginTop, marginBottom, onClickArr, className}) => {
...
const createStep1 = useCallback((v, i) => {
  return (
    <>
      <p>{transformText(v)}</p>
      ...
      {subContents !== undefined && <p className="sub-contents">{transformText(subContents[i])}</p>}
    </>
  );
}, []);
const createStep2 = useCallback((v, i) => <><p>{transformText(v)}</p><div className="step-ico">{
  !hasMobile
    ? type === 1 ? i+1 : null
    : type === 2 ? `0${i+1}` : i+1
}</div>{subContents !== undefined && <p className="sub-contents">{transformText(subContents[i])}</p>}</>, []);
    ...
    return (
      <div className="steps-stick">
        <div className="step-line">
          <span style={{width: `${active*(100/contents.length)}%`}}></span>
        </div>
        <p>{contents[active-1]} ({active}/{contents.length} 단계)</p>
      </div>  
    )
  }
