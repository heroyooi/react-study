...
let checkArr = [];
const OrganizationInfo = (response) => {
  ...
  const { chkNmMsg, chkEmlMsg, chkBlFileMsg, chkMgmtBlFileMsg, chkCorpcertFileMsg, chkEntrAddrMsg, chkAdmnEntrMsg, chkSctrsNmMsg } = msgs2;
  ...
  const { mbId, mbPwdEnc, mbPwdEncChk, admnEntrRegNo, sctrsNm, mbEmlAddrEnc, entrTpCd, entrZcd, entrAddrEnc, entrDtlAddrEnc, mbNm } = inputs;
    ...
    if (idValidation(e) && pwdValidation(e)) {
      if (entrTpCd === '0010' && isEmpty(admnEntrRegNo)) {
        ...
        if (inputs.blFileIdList === undefined) {
          ...
          // eslint-disable-next-line no-dupe-keys
          setMsgs2({ chkCorpcertFileMsg, ['chkCorpcertFileMsg']: '' });
          return true;
        }
      }
    }
        ...
        axiosPost('/api/member/insertMemberFile.do', fileData).then(({ data }) => {
          ...
          if (data.statusinfo.returncd === 'SUCCESS') {
            ...
            axiosPost('/api/member/insertMemberInfo.do', paramObj, false).then(({ data }) => {
              ...
              if (data.statusinfo.returncd === 'SUCCESS') {
                /*
                Router.push(`/member/memberRegistrationComplete?mbTpcd=0030`).then(() => {
                  window.scrollTo(0, 0);
                });*/
                window.location.href = '/member/memberRegistrationComplete?mbTpcd=0030';
                window.scrollTo(0, 0);
              } else {
  ...
  if (hasMobile) {
    ...
    const [fpAddress, setFpAddress] = useState(false); // 약관 팝업
    ...
    return (
      <AppLayout>
        ...
        <div className="member-sec">
          <div className="content-wrap member-contents bg-white">
            <form className="join-form">
              <fieldset>
                <legend className="away">가입정보입력</legend>
                {/* 회원 기본 */}
                <JoinBaseInfo mbInfo={mbInfo} onChange={onChange} msgObj={msgs} onBlur={onBlur} mbTpcd={'0030'} onClick={onIdDup} />

                {/* 단체/제휴 기본 */}
                {mbInfo.rtnMbInfo && <JoinSocietyInfo mbInfo={mbInfo} onChange={onChange} msgObj={msgs2} onBlur={onBlur} mbTpcd={'0030'} />}
              </fieldset>
            ...
            <Button className="fixed" size="full" background="blue80" title="가입완료" href="/member/memberRegistrationComplete" nextLink={true} />
          </div>
        </div>
...
OrganizationInfo.getInitialProps = async (http) => {
  /*
  console.log("BACK 필요!!!!!")
  const { req } = http;
  const data = req.query;
  //console.log("OrganizationInfo:", req.query);
  const res = await 
