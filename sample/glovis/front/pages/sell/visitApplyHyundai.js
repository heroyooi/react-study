...
const VisitApplyHyundai = () => {
  ...
  useEffect(() => {
    ...
    axiosGet('/api/InfoPolicy/selectInfoPolicy.do?tmsId=AA000010&tmsTp=0010').then((res) => {
  ...
  const handleTermsPopup = useCallback(
    (e) => {
      e.preventDefault();
      handleFullpagePopup('terms')(e);
    },
    []
  );
  ...
  const onClickSummit = useCallback(
    (e) => {
      ...
      axiosPost('/api/pricing/insertVisitApply.do', inputData)
        .then((res) => {
          if (res && res.data && (res.data.statusinfo.returncd === 'SUCCESS' || res.data.statusinfo.returncd === '000')) {
            if (hasMobile) {
              //Router.push('/sell/visitcomplete');
              alert('방문신청 요청이 처리되었습니다.');
              window.location.href = 'https://www.hyundai.com/';
            } else {
              alert('방문신청 요청이 처리되었습니다.');
              //setRodalShow4(false);
              //Router.push('/sell/visitcomplete');
    ...
    [selectedLocation, selectedCity, nameValue, selValue, mobileValue, birthYearValue, birthMonthValue, birthDayValue, carNumValue, carNameValue, hasMobile, setRodalShow4]
  );
      ...
      <AppLayout>
        <div className="content-sec">
          ...
          <MobBottomArea active={applyTerms} isSimple={true} mode="fade">
            <MobSelectTerms onClick={onClickRequest} onTermsClick={handleTermsPopup} termsData={[{ id: 'chk1', title: '방문평가 이용약관 (필수)', checked: false }]} />
          </MobBottomArea>
            ...
            <div className="member-terms-wrap">
              <div className="view-wrap">
                <div className="content">
                  <div className="agrscr" id="priv_frm">
                    <div className="privcon">
                      <p className="pt">개인정보 수집·이용 및 제 3자 제공 동의서</p>      
                      <p className="pss">
                      현대글로비스는 내차팔기 상담 서비스 제공을 위하여 아래와 같이 개인정보를 수집·이용·제공하고자 합니다.
                      처리한 개인정보는 목적 이외의 용도로는 사용되지 않으며 이용 목적이 변경될 시에는 사전동의를 구할 예정입니다.<br /><br />
                      ■ 개인정보 수집·이용 내역<br />
                      </p>
                      <table border="1" style={{textAlign:'center'}}>
                        ...
                        <tr>
                            <td>이름, 전화번호, 차량번호</td>
                            <td>내차팔기 상담 서비스 제공 등</td>
                            <td>1년</td>
                        </tr>
                      </tbody>
                      </table>

                      <p className="pss"> ■ 개인정보의 제3자 제공<br />
                      </p><table border="1" style={{textAlign:'center'}}>
                        <colgroup>
                          <col width="25%" />
                          <col width="25%" />
                          <col width="25%" />
                          ...
                          <tr>
                              <td>(주)제츠 외 68개</td>
                              <td>차량평가 및 매각상담 이행</td>
                              <td>이름, 전화번호, 차량번호</td>
                              <td>1년</td>
                          </tr>
                          <tr>
                              <td>타미마케팅(주), 오토앤, (주)엠트웰브</td>
                              <td>매각 고객 대상 경품 지급</td>
                              <td>이름, 전화번호, 차량번호</td>
                              <td>1년</td>
                          </tr>
                      </tbody></table>
                      {/* <a style={{textDecoration:'underline',marginBottom:'30px'}} href="https://www.autobell.co.kr:444/cm/fileDownMan.do?menuCd=PDF&amp;filename=FkCNG%2FoVQG91nqIVFX0hoHnix3WMacX0ycCjVlyld6Q%3D" target="_blank">매집평가사 전체보기</a>
                      <br /><br /><br /> */}
                      <p className="pss"> ■ 개인정보 취급업무 위탁 내역<br />
                      </p><table border="1" style={{textAlign:'center'}}>
                        <colgroup>
                          <col width="25%" />
                          <col width="25%" />
                      ...
                      <p className="pss">위와 같은 개인정보 수집·이용·제공에 대한 동의를 거부할 권리가 있습니다. 그러나 동의를 거부할 경우 원활한 서비스 제공에 일부 제한을 받을 수 있습니다.</p>
                    </div>
                  </div>
                </div>
  ...
  return (
    <FrameLayout>
      <div className="visual-area">
        <div className="cont">
          <span className="c1">
            <img src="/images/contents/hyundai-motor.png" alt="HYUNDAI" />
          </span>
          <span className="c2">
            <img src="/images/contents/x.png" alt="X" />
          </span>
          <span className="c3">
            <img src="/images/contents/autobell-logo.png" alt="autobell" />
          </span>
          <p>
            <strong>현대자동차</strong>와 함께하는
            <br />
            <strong>현대글로비스</strong>의 내차팔기 서비스
          </p>
        </div>
      </div>
      <div className="content-wrap visit-apply-wrap">
        <div className="visit-img-sec">
          <h3 className="visit-tit">방문평가판매</h3>
          <p className="visit-copy">
            간편 신청을 통해 전담 차량 평가사가 방문하여
            <br /> 차량 평가에서 매각까지 도와드립니다.
          </p>
      ...
      <RodalPopup show={rodalShow1} type={'fade'} closedHandler={modalCloseHandler1} title="방문평가 이용약관 (필수)" mode="normal" size="medium">
        {/* <div className="con-wrap" dangerouslySetInnerHTML={{ __html: termCont ? termCont.tmsCntn || '' : '' }} /> */}
