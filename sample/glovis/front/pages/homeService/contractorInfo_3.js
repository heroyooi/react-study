...
const ContractorInfo_3 = ({ query }) => {
  ...
  const handleCorpRegInput = (value) => {
    const corpRegNumRegexWhen5 = /^([0-9]{6})([0-9]*)$/;
    value = value.replace(/-/gi, '');

    if (value.length < 7) return value;
    if (value.length >= 7) return value.replace(corpRegNumRegexWhen5, '$1-$2');
  ...
  const [isHp1Check, setIsHp1Check] = useState(false); //휴대폰
  const [isAddr1Check, setIsAddr1Check] = useState(false); //법인 주소
  const [isAddr2Check, setIsAddr2Check] = useState(false); //배송지 주소

  const validationChk = () => {
    if (formData.corpNm === '') {
      showAlert('법인명을 입력하세요.', 'error');
      return;
    } else if (formData.corpRegNo === '') {
              ...
              <tbody>
                <tr>
                  <td>
                    ...
                    <Input id="corpNm" value={formData.corpNm} type="text" onKeyUp={(e) => handleOnKeyUp(e, 'corpNm')} onChange={onChangeInput} maxLength={20} />
                  </td>
                </tr>
                <tr>
                  <td>
                    ...
                    <Input
                      type="text"
                      id="corpRegNo"
                      value={formData.corpRegNo ? handleCorpRegInput(formData.corpRegNo) : ''}
                      onChange={onChangeRemoveFormatInput}
                      onKeyUp={(e) => handleOnKeyUp(e, 'corpRegNo')}
                      maxLength={14}
                    />
                  </td>
                </tr>
                  ...
                  <td>
                    ...
                    <span className="bridge2">
                      <Input
                        ...
                        type="text"
                        width="100%"
                        value={formData.brn1 ? handleBizNumInput(formData.brn1) : ''}
                        onKeyUp={(e) => handleOnKeyUp(e, 'brn1')}
                        onChange={onChangeRemoveFormatInput}
                        maxLength={12}
                      />
                ...
                <tr>
                  <td>
                    ...
                    <span className="bridge2">
                      <Input id="corpZcd" type="text" placeHolder="우편번호" disabled={true} width="73%" value={formData.corpZcd ? formData.corpZcd : ''} onChange={onChangeInput} />
                      <Button size="mid" background="blue80" radius={true} title="우편번호" measure={true} width={24.5} onClick={handleFullpagePopup('Address', 'corpAddr')} />
                    </span>
                    <span className="bridge2">
                      <Input id="corpAddr1" type="text" disabled={true} placeHolder="주소" value={formData.corpAddr1 ? formData.corpAddr1 : ''} onChange={onChangeInput} />
                    </span>
                    <span className="bridge2">
                      <Input id="corpAddr2" type="text" placeHolder="상세주소" value={formData.corpAddr2 ? formData.corpAddr2 : ''} onChange={onChangeInput} />
                    </span>
                    {!formData.corpAddr2 && isAddr1Check && <p className="tx-sub tx-red80">상세주소를 입력해주세요.</p>}
                  </td>
                </tr>
                <tr>
                  <td>
                ...
                <tr>
                  <td>
                    <p className="tx-tit">
                      배송지주소
                      <CheckBox className="fr" id="chk-address" title="법인주소와 동일" onChange={(e) => onChangeCheck(e)} />
                    </p>
                    <span className="bridge2">
                      <Input id="zcd1" type="text" value={formData.zcd1 ? formData.zcd1 : ''} disabled={true} placeHolder="우편번호" width="73%" onChange={onChangeInput} />
                    ...
                    <span className="bridge2">
                      <Input id="addr1" type="text" disabled={true} placeHolder="주소" value={formData.addr1 ? formData.addr1 : ''} onChange={onChangeInput} />
                    </span>
                    <span className="bridge2">
                      <Input id="addr2" type="text" placeHolder="상세주소" value={formData.addr2 ? formData.addr2 : ''} onChange={onChangeInput} />
                    </span>
                    {!formData.addr2 && isAddr2Check && <p className="tx-sub tx-red80">상세주소를 입력해주세요.</p>}
                  </td>
                </tr>
                <tr>
                  <td>
                ...
                <tr>
                  <td>
                    차량양도계약서 서명방식
                    <RadioGroup
                      dataList={[{ id: 'radio_signature', value: '0010', checked: false, disabled: false, label: '직접서명' }]}
...
ContractorInfo_3.getInitialProps = async (http) => {
...
export default withRouter(ContractorInfo_3);
