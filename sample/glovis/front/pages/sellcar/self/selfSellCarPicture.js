...
// Image Editor
import ImgEditor from '@src/components/common/imageEditor/imageeditor.js';

const nextPage = `/sellcar/self/selfSellCarConfirm`;
const prevPage = `/sellcar/self/selfSellCarInfo`;

/**
 * 셀프 판매 서비스 차량 사진 등록
 * @returns {selfSellCarInfo}
 */
const selfSellCarPicture = ({ query }) => {
  ...
  const [imgEditorPop, setImgEditorPop, showImgEditorPop, hideImgEditorPop] = useRodal(false, true);
  ...
  const sendData = async (e) => {
    ...
    formData.append('hpPn', seller.hpPn);
    formData.append('slReqId', seller.slReqId);
  ...
  const goNextStep = async (e) => {
    ...
    showConfirm(`저장 후, 다음단계로 이동하시겠습니까?`, async () => {
      ...
      if (result) {
        ...
        showAlert(`저장중 에러가 발생했습니다. 로그인 이후 다시 진행부탁드립니다.`);
      }
    });
  };
  ...
  return (
    <AppLayout>
      <div className="content-wrap sell-fore-wrap">
        <h3><span className="stp"><b>비교견적</b>으로 내 차 팔기</span>Step 3</h3>
      </div>
            ...
            <Button
              ...
              onClick={() => {
                showConfirm('이전 단계로 이동하시겠습니까?', () => {
                  Router.push(`/sellcar/self/selfSellCarInfo?slReqId=${slReqId}`);
                });
              }}
            />
          </span>
      ...
      {/* Image Editor */}
      <RodalPopup show={imgEditorPop} type={'fade'} closedHandler={hideImgEditorPop} mode="normal">
        <ImgEditor />
      </RodalPopup>
    </AppLayout>
  );
...
selfSellCarPicture.getInitialProps = async (http) => {
...
export default selfSellCarPicture;
