...
import { getLocCodeList, getMonthlyInstallList, getTotalCostCalData, getCarTaxCalData, yearList } from '@src/actions/buycar/totalCost/totalCostAction';

const TotalCostPopup = ({ costPopupShow, setCostPopupShow, closeCostPopup, BuyViewInfo }) => {
  console.log('BuyViewInfo => ', BuyViewInfo);
  ...
  const [carPrice, setCarPrice] = useState(BuyViewInfo.carPrice * 10000); // 차량 가격@@@@@@
  ...
  const [yy, setYy] = useState(Number(BuyViewInfo.yy)); // 차량 연식@@@@@@
  ...
  //   { value: '2009', label: '2009년' },
  //   { value: '2008', label: '2008년' },
  //   { value: '2007', label: '2007년' },
  //   { value: '2006', label: '2006년' },
  ...
  // 용도 라디오 리스트
  const usageList = [
    { id: 'non-business', value: '2', checked: true, disabled: false, title: '비영업용' },
    { id: 'business', value: '1', checked: false, disabled: false, title: '영업용' }
  ];

  // 등록구분 라디오 리스트
  const enrollList = [
    { id: 'first-half', value: '1', checked: true, disabled: false, title: '1/1~6/30등록차량' },
    { id: 'second-half', value: '2', checked: false, disabled: false, title: '7/1~12/31등록차량' }
  ];
  ...
  // 계산하기 버튼
  const ClickCalculate = useCallback(
    (e) => {
      e.preventDefault();
      ...
      if (enroll === '2') {
        if (formDt < Fday) {
          ...
          setCarOld12(sumYear + 1);
          const DateForm = moment.duration(Fday.diff(moment(carAssignDt, 'MM-DD'))).asDays();
          // console.log('test test1 ---->> ', Math.ceil(DateForm));
          // console.log('test test2 ---->> ', 181 - Math.ceil(DateForm) - 186);
    ...
    [transmitterDt, assigneeDt, assignChk, enroll, dspl, ccAmt, alleviateRt, alleviateRt12, carAssignDt]
  );
  ...
  // 총비용 -- 이전등록비 합계
  useEffect(() => {
    setTransRegistFee(registTax + acquTax + pbndByg + 1000 + 3000);
    // console.log(registTax, acquTax, pbndByg);
  ...
  //                 <SelectBox id="area" className="items-sbox" options={locList} width="100%" height={48} value={locCd} onChange={onChangeLocCd} />
  //               </div>
  ...
  //                       <th>차량가격</th>
  //                       <td>
  //                         <span>{numberFormat(carPrice)}</span>원
  //                       </td>
  //                     </tr>
  //                   </tbody>
  //                 </table>
  ...
  //                       <th>이전등록비 합계</th>
  //                       <td>
  //                         <span>{numberFormat(transRegistFee)}</span>원
  //                       </td>
  //                     </tr>
  //                     <tr>
  //                       <th>등록세({regRt}%)</th>
  ...
  //                       <th>총 구매 예상 비용</th>
  //                       <td>
  //                         <span>{numberFormat(carPrice + transRegistFee)}</span>원
  //                       </td>
  //                     </tr>
  //                   </tbody>
  //                 </table>
  //                 <div className="ico-wrap">
  //                   <i className="ico-circle-plus" />
  //                   <i className="ico-circle-equal" />
  //                 </div>
  //               </div>
  //               <Buttons align="left" marginTop={25}>
  //                 <span className="fr">
  //                   <Button size="mid" line="gray" radius={true} title="보험료 조회" width={102} buttonMarkUp={true} onClick={ClickPremium} />
  //                   <Button size="mid" line="gray" radius={true} title="할부상품 조회하기" width={144} buttonMarkUp={true} onClick={ClickInstallProduct} />
  //                 </span>
  //               </Buttons>
  //             </TabCont>
  ...
  //                   <SelectBox id="period" className="items-sbox" options={monthlyList} width={248} height={48} value={period} onChange={onChangePeriod} />
  //                 </li>
  //                 <li className="fr">
  //                   <label htmlFor="percent">선수율</label>
  //                   <SelectBox id="percent" className="items-sbox" options={percentList} width={248} height={48} value={percent} onChange={(e) => setPercent(e.value)} />
  //                 </li>
  //               </ul>
  ...
  //                       <th>총 할부 금액</th>
  //                       <td>
  //                         <span>{numberFormat(totalCost)}</span>원
  //                       </td>
  //                     </tr>
  //                     <tr>
  //                       <th>할부원금</th>
  ...
  //                   <tbody>
  //                     <tr>
  //                       <th>월 할부 예상 금액({periodDate}개월)</th>
  //                       <td>
  //                         <span>{numberFormat((totalCost / periodDate).toFixed(0))}</span>원
  //                       </td>
  //                     </tr>
  //                   </tbody>
  //                 </table>
  ...
  //                 <span className="fr">
  //                   <Button size="mid" line="gray" radius={true} title="다른 할부상품으로 다시 조회하기" width={237} buttonMarkUp={true} onClick={ClickOtherProduct} />
  //                 </span>
  //               </Buttons>
  //             </TabCont>
  ...
  //                         <th>용도</th>
  //                         <td>
  //                           <RadioGroup dataList={usageList} defaultValue={usage} onChange={(e) => setUsage(e.target.value)} />
  //                         </td>
  //                       </tr>
  //                       <tr>
  //                         <th>등록구분</th>
  //                         <td className="pd8-12">
  //                           <RadioGroup dataList={enrollList} defaultValue={enroll} onChange={(e) => setEnroll(e.target.value)} />
  //                         </td>
  //                       </tr>
  //                       <tr>
  //                         <th>
  //                           <label htmlFor="year">연식/배기량</label>
  //                         </th>
  //                         <td>
  //                           <SelectBox id="year" className="items-sbox" options={yearList} width={120} height={38} value={yy} onChange={(e) => setYy(e.value)} />
  //                           <em className="mg8">/</em>
  //                           <Input type="text" id="engine-cc" placeHolder="1951" width={120} height={38} value={dspl} onChange={(e) => setDspl(e.target.value)} />
  //                           <em>CC</em>
  //                         </td>
  //                       </tr>
  //                       <tr>
  //                         <th>일할계산여부</th>
  //                         <td>
  //                           <RadioGroup dataList={calculateChkList} defaultValue={calculateChk} onChange={(e) => setCalculateChk(e.target.value)} />
  //                         </td>
  //                       </tr>
  //                       {calculateChk === '1' && (
  //                         <>
  ...
  //                             <th>차량양도일</th>
  //                             <td>
  //                               <DatePicker defaultValue={carAssignDt} inputWidth={245} inputHeight={38} onChange={(e) => setCarAssignDt(e)} />
  //                             </td>
  //                           </tr>
  //                           <tr>
  //                             <th>양수/양도인</th>
  //                             <td>
  //                               <RadioGroup dataList={assignChkList} defaultValue={assignChk} onChange={(e) => setAssignChk(e.target.value)} />
  //                             </td>
  //                           </tr>
  //                         </>
  //                       )}
  ...
  //                     <Button size="big" line="black" color="black" title="계산하기" width={180} buttonMarkUp={true} onClick={ClickCalculate} />
  //                   </Buttons>
  //                 </TabCont>
  ...
  //                     <br />※ 위 금액은 참고자료이며 정확한 금액은 해당 지역 세무 담당 부서에 문의하세요.
  //                   </p>
  //                 </TabCont>
  ...
  //             <p className="tx-sub">
  //               위 금액은 예상비용으로 실제 금액과 상이할 수 있으며, 경우에 따라 매도비용,
  //               <br />
  //               알선수수료가 발생할 수 있습니다. 실제 거래신고 금액이 과세표준금액보다 적으면,
  //               <br />
  ...
  //               <li>
  //                 수소전기차 : 취득세 최대 140만원 감면, 공채 매입 최대 250만원 면제
  //                 <br />
  //                 (2019년 7월 23일 기준입니다.)
  //               </li>
  //             </ul>
  ...
  //         calculateChk === '1' &&
  //         dep1Panel === 2 &&
  //         dep2Panel === 0 && ( // 자동차세 탭의 일반승용차 탭 활성화 시
  //             <div className="popup-bg bottom">
  //               <p className="tx-tit">
  //                 계산결과{' '}
  //                 <span>
  //                   (과세년도 : {moment().format('YYYY')}년 / cc당 세액 : {ccAmt}원)
  //                 </span>
  //               </p>
  ...
  //                   <tr>
  //                     <td>1년분</td>
  //                     <td>{numberFormat(CarP6 + CarP12 + AduP6 + AduP12)} 원</td>
  //                     <td>{numberFormat(CarP6 + CarP12)} 원</td>
  //                     <td>{numberFormat(AduP6 + AduP12)} 원</td>
  //                     <td>{carOld6} 년</td>
  //                     <td>{!enrollChk ? alleviateRt : alleviateRt12}%</td>
  //                   </tr>
  //                   <tr>
  //                     <td>1기분(6월)</td>
  //                     <td>{numberFormat(CarP6 + AduP6)} 원</td>
  ...
  //                   <tr>
  //                     <td>2기분(12월)</td>
  //                     <td>{numberFormat(CarP12 + AduP12)} 원</td>
  ...
  //                     <td>
  //                       <span className="tx-blue80">{numberFormat(finalCarP + finalAduP)}</span> 원
  //                     </td>
  //                     <td>
  //                       <span className="tx-blue80">{numberFormat(finalCarP)}</span> 원
  //                     </td>
  //                     <td>
  //                       <span className="tx-blue80">{numberFormat(finalAduP)}</span> 원
  //                     </td>
  //                     <td>
  //                       <span className="tx-blue80">{carOld12}</span> 년
  //                     </td>
  //                     <td>
  //                       <span className="tx-blue80">{alleviateRt12}</span>%
  //                     </td>
  //                   </tr>
  //                   <tr>
  //                     <td colSpan="5" className="tx-l">
  //                       일할 계산 된 자동차 보유일 수: {assignChkCal ? transmitterDt : assigneeDt}일
  //                     </td>
  //                   </tr>
  //                 </tbody>
  //               </table>
  //               <div className="popup-bg bottom">
  //                 <p className="tx-tit">꼭 알아두세요!</p>
  //                 <p className="tx-sub">위 금액은 예상비용으로 실제 금액과 상이할 수 있습니다.</p>
  //               </div>
  //             </div>
  //           )}
  //         {!calculate &&
...
export default TotalCostPopup;

TotalCostPopup.propTypes = {
  costPopupShow: PropTypes.bool,
  setCostPopupShow: PropTypes.func,
  closeCostPopup: PropTypes.func,
  BuyViewInfo: PropTypes.object
};
