...
const nextPage = `/mypage/dealer/autoauction/registrationComplete`;

const memberInfo = ({ mbCiO, mbNmO, mbHpPnEncO, mbBirth, memberinfo = {} }) => {
  console.log('mbNmO : ', mbNmO)
  console.log('mbHpPnEncO : ', mbHpPnEncO)
  ...
  const mbTpcd = useSelector((RootStore) => RootStore.member);
  const { auctMbInfo, auctMbCorpInfo } = auctionMember;
  const [agreeUsingAccount, setAgreeUsingAccount] = useState(false);

  console.log('memberInfo -> auctionMember : ', auctionMember);
  // console.log("memberInfo -> query", query)
  ...
  const goNextStep = (e) => {
    ...
    if (error) {
      ...
      showConfirm('저장 후, 다음단계로 이동하시겠습니까?', async () => {
        ...
        const { data, statusinfo } = await dispatch(
          postMyAuctionMemberInfoAction({
            ...addMbInfo,
            ...addStoreInfo
          })
        ).then((res) => res?.data);
      ...
      <div className="content-wrap">
        ...
        <div className="mypage-state-sec">
          <div className="tabmenu tp1">
            ...
            {/* <form>
              <input type="text" name="mbCiO" value={mbCiO} />
              <input type="text" name="mbNmO" value={mbNmO} />
              <input type="text" name="mbHpPnEncO" value={mbHpPnEncO} />
            </form> */}
...
memberInfo.getInitialProps = async (http) => {
  ...
  return {
    memberinfo: data[0],
    mbCiO, mbNmO, mbHpPnEncO, mbBirth
  };
};
