...
const confirmPwd = () => {
  ...
  const handleChkPwd = useCallback((e) => {
    e.preventDefault();
    if (isEmpty(mbPwdEnc)) {
    ...
    dispatch(getMemberInfoPwd(paramObj));
  });

  useEffect(() => {
    if (memberInfoPwd === null) {
      setChkMsg('비밀번호가 일치하지 않습니다.');
    } else if (memberInfoPwd.mbId === userId) {
      //Router.push('/mypage/dealer/info/changePwd');
      window.location.href = '/mypage/dealer/info/changePwd';
    }
  }, [memberInfoPwd]);
