...
//UpdateTicketList 컴포넌트와 완전 동일한 컴포넌트지만 추후 어떻게 될지 모르기에 둘로 나눠둠
const FreeTicketList = ({ items, onChange, checkedValue, checkableProps, selectable = false, usingTicketList, subscribe }) => {
  const hasMobile = useSelector((state) => state.common.hasMobile);
  const [ticketNum, setTicketNum] = useState(1);
  const ticketList = useMemo(() => {
    return monthList.reduce((list, { month }) => {
      console.log('free list : ', items);
      //1,3,6,12 개월 순으로 담는다. //crSlot
  ...
  const ticketChange = useCallback((e) =>{
    if(e.target.value !== undefined){
      setTicketNum(Number(e.target.value));
    }
  },[])
  // 모바일 컴포넌트
  if (hasMobile) {
      ...
      return (
        <>
          ...
          <div className="content-border pd20 mt24">
            <p className="tit4">자유이용권 1개월 10대</p>
            <ul className="info">
              <li>
...
export default memo(FreeTicketList)