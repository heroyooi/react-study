...
const MobFilterEngine = ({ displayMemberPath, itemsSource, internalSelectedItems, selectedValuePath, onChange, onSelect }) => {
  return (
    <>
          ...
          <ul>
            {(itemsSource || []).map((data, idx) => {
              return (
                <li key={idx}>
                  <CheckBox
                    id={`chk-engine-${data[selectedValuePath]}`}
...
export default withMobFilterPopUp(MobFilterEngine);
