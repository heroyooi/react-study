...
/**
 * 04.01 _SlideGallery 와 통합
 *
 * html 변경이력
 * 03.16 : [모바일] callback props 추가 및 클릭 이벤트 처리, #a1 부분 참고 -> 03.28 반영
 */
const SlideGallery = memo(({ car_gallery, quickView = false, callback }) => {
  ...
  let settings_root_slick = {
    ...
    customPaging: function(i) {
      ...
      return (
        <a>
          <ImgCover src={`${imgUrl}${sImg}`} alt={sAlt} />
        </a>
      );
    },
    ...
    return (
      ...
      >
        {car_gallery?.map((value, index) => {
          return (
            <div key={index}>
              <ImgCover src={`${imgUrl}${value.bImg}`} alt={value.bAlt} />
    ...
    <div className="car-img-area">
      ...
      <Slider ref={rootSlider} {...settings_root_slick}>
        {car_gallery?.map((value, index) => {
          return (
            <div key={index}>
              <a href="" onClick={!hasMobile ? (e) => rodalPopupHandler(e, 'fade') : (e) => handleClick(e, index)}>
                {/* #a1 */}
                <ImgCover src={`${imgUrl}${value.bImg}`} alt={value.bAlt} />
                {value.options && value.options.length > 0 && (
                  <div className="price-opt">
      ...
      <RodalPopup show={rodalShow} type={rodalType} maxWidth={1200} width={90} closedHandler={modalCloseHandler} mode="fullsize" measure="%">
        <Slider ref={popupSlider1} asNavFor={popupNav2} prevArrow={<PrevArrow />} nextArrow={<NextArrow />} slidesToShow={1} fade={true} adaptiveHeight={true} {...settings_popup_slick}>
          {car_gallery.map((v, i) => {
            return <Magnifier key={i} imgUrl={`${imgUrl}${v.bImg}`} alt={v.bAlt} />;
          })}
        </Slider>
        <Slider
          ref={popupSlider2}
          ...
          {car_gallery.map((v, i) => {
            return (
              <div key={i} style={{ width: 50 }}>
                <ImgCover src={`${imgUrl}${v.bImg}`} alt={v.bAlt} />
              </div>
            );
          })}
        </Slider>
