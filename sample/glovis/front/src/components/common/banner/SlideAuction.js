  ...
  return (
    <>
      <div className={slider_style}>
        <Slider {...settings}>
          {car_list.map((v, i) => (
            // <ImgCover key={i} src={`${imgUrl}${v.bImg}`} alt={v.bAlt} />
            <ImgCover key={i} src={v.bImg} alt={v.bAlt} />
          ))}
        </Slider>
      ...
      {!hasMobile && (
        <div className="auction-exp">
          <p className="tx-exp-tp2">* 현대 글로비스 스마트옥션을 통한 믿을 수 있는 경매 낙찰 차량 입니다.</p>
          <p className="pagination-num">
            <span>{slideIndex + 1}</span>/{car_list.length}
          </p>
        </div>
      )}
    </>
