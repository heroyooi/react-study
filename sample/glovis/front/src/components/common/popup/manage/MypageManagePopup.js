...
import MypageSellPrice from '@src/components/common/manage/MypageSellPrice';
...
import { axiosGet, axiosPost, axiosDown } from '@src/utils/HttpUtils';
...
const MypageManagePopup = () => {
  ...
  const {
    carDeleteShow,
    carDeleteCloseHandler,
    carDeletePopupHandler,
    rodalShow,
    modalCloseHandler,
    rodalPopupHandler,
    rodalShow1,
    ...
    rodalShow9_1,
    setRodalShow9_1,
    modalCloseHandler9_1,
    rodalPopupHandler9_1,
    rodalShow10,
    modalCloseHandler10,
    rodalShow11,
    modalCloseHandler11,
    rodalShow11_1,
    modalCloseHandler11_1,
    rodalShow12,
    modalCloseHandler12,
    rodalShow13,
    modalCloseHandler13,
  ...
  const changeInput = (e, item) => {
    ...
    setCarInfo(
      produce((draft) => {
        draft['carNumber'] = e.target.value;
      })
    );
  };

  // 차량정보 수정 버튼 이벤트 핸들러
  const handleModify = (e, carNumber, carTypeYear) => {
    e.preventDefault();
    console.log(carInfo);

    const url = `/api/mypage/dealer/prodCarInfoUpdate.do`;
    axiosPost(url, carInfo).then((res) => {
      setRodalShow9(false);
      setRodalShow9_1(false);
    });
  };
  ...
  // 차량가격 수정 버튼 이벤트 핸들러
  const save = (e, slAmt, dlrPrdId) => {
    e.preventDefault();
    console.log('carInfo : ', carInfo);
    let resultMsg = '수정되었습니다.';

    const url = `/api/mypage/dealer/updateMyCarPrice.do?slAmt=${slAmt}&dlrPrdId=${dlrPrdId}`;
    axiosPost(url, carInfo).then((res) => {
      showAlert(resultMsg);
    });
  };
    ...
    <>
        ...
        <div className="popup-update">
          <div className="top">
            <p>
              해당 차량을 업데이트 하시겠습니까?
              <br />
              <span>(업데이트 횟수를 모두 사용한 차량 및 자동업데이트 차량은 제외됩니다.)</span>
            </p>
          </div>
          <div className="bot">
            <p className="tx-btn-wrap">
              더 많은 횟수와 자동으로 되는 업데이트가 필요하세요?
              <Button size="mid" background="blue80" radius={true} title="자세히 보기" width="104" />
              <br />
              <span>뒤로 밀린 차량을 리스트 가장 앞으로 매시간 자동 이동해주는 신규 상품 출시!</span>
            </p>
            <div className="update-consult">
              <i className="ico-ad"></i>
              <p>
                혹시 지금 구매자들이 많은 시간대가 아닌데 업데이트 하시나요?
                <br />
                광고노출이 가장 효과적인 시간에 업데이트 하세요.
                <br />
                <span>1577-9789</span>(무료 컨설팅)
              </p>
            </div>
            <Buttons align="center" marginTop={48}>
              <Button size="big" background="gray" title="취소" width={172} height={60} />
      ...
      <RodalPopup show={rodalShow8} type={'fade'} closedHandler={modalCloseHandler8} title="가격 수정" mode="normal" size="medium">
        <div className="con-wrap price-modify">
          ...
          <Buttons align="center" marginTop={48}>
            <Button size="big" background="gray" title="취소" width={172} height={60} />
            <Button size="big" background="blue80" title="수정" width={172} height={60} onClick={save} />
          </Buttons>
        </div>
      </RodalPopup>
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      ...
                      <Input type="text" value={carInfo.carNumber} id="car-num" width={160} height={40} onChange={changeInput} />
                    </td>
                    <th>주행거리</th>
                    <td>
                      <Input type="text" value="84,761km" id="engine-cc" width={160} height={40} />
                    </td>
                  </tr>
                  <tr>
                    <th>최초등록일</th>
                  ...
                  <tr>
                    <th>색상</th>
                    <td>
                      <SelectBox id="car-color" className="items-sbox" options={[{ value: '1', label: '검정' }]} width={160} height={40} placeHolder="검정" />
                    </td>
                    <th>변속기</th>
                    <td>오토</td>
                  </tr>
        ...
        <div className="con-wrap popup-perform-record">
          <div className="performance-chk">
            <ul className="pt-number">
              <li>제시번호</li>
              <li>
                <Input type="text" id="Propose01" width={262} />
              </li>
              ...
              <table summary="특기사항 및 점검자의 의견에 대한 내용" className="table-tp1 input">
                ...
                <tbody>
                  <tr>
                    ...
                    <td>성능•상태 점검자</td>
                    <td>
                      <Input type="text" id="expert-opinion" disabled={true} />
                    </td>
                  </tr>
                </tbody>
              </table>
      ...
      <RodalPopup show={rodalShow14} type={'fade'} closedHandler={modalCloseHandler14} title="영수증/증빙" mode="normal" size="small">
        <div className="con-wrap popup-payment">
          ...
          <p>
            발급영수증 선택(결제후 다음달 5일까지)
            <br />* 현금영수증/세금계산서 중 <b>1개</b>만 증빙자료로 신청이 가능합니다.
          </p>
        ...
        <div className="con-wrap popup-no-group-id">
          ...
          <RadioGroup
            dataList={[
              ...
              { id: 'radio4', value: 2, checked: false, disabled: false, title: '금융사제휴회원' },
