...
const CarColorPicker = ({ isTitle = true, selectedColor, selectedSubColor, onClose, onClick, mode = 'check', isSelectButton = true }) => {
  ...
  const handleClick = useCallback(
    (e) => {
      e.preventDefault();
      onClick && onClick(e, { selectColor, selectColorCode, selectSubColor, selectSubColorCode });
    },
    [onClick, selectColor, selectColorCode, selectSubColor, selectSubColorCode]
  );
  ...
  const handleChangeSub = useCallback((e) => {
    setSelectSubColor(e.target.name);
    setSelectSubColorCode(e.target.value);
  }, []);
                ...
                return (
                  <li key={index}>
                    <CheckBox
                      ...
                      name={item.label}
                      checked={selectSubColor === item.value}
                      isSelf={modeCheck}
