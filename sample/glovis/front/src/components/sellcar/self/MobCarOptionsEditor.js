...
const CarOptionsEditor = ({ items, isEditing, onCheck, addOptions, className, popup = false }) => {
  ...
  const clickLi = (e, option) => {
    ...
    console.warn(e.target.tagName, option);

    if (tagName == 'I' || tagName == 'SPAN' || tagName == "LI") {
        ...
        <ul className="cate-ico">
          {isEditing
            ? carOptions?.map((option, i) => {
                return (
                  <li className={classNames({ on: isExist(option.optCd) })} key={i} onClick={(e) => clickLi(e, option)}>
                    ...
                    <i className={`ico-${iconClass[option.optCd]}`} />
                    <span className="ico-exp" dangerouslySetInnerHTML={{ __html: changeOptNm(option.optNm) }} />
                  </li>
                );
              })
            : carOptions?.map((option, i) => (
                <li className={classNames({ on: isExist(option.optCd) })} key={i}>
                  <Tooltip placement="bottomLeft" width={400} zid={101}>
                    <TooltipItem>
                      <i className={classNames({ [`ico-${iconClass[option.optCd]}`]: true })} />
                      <span className="cate-ico-exp" dangerouslySetInnerHTML={{ __html: option.optNm }} />
                    </TooltipItem>
                    <TooltipCont>
                      <div className="cate-list-tooltip img-none">
                        <img src={option.imgUrl} alt={option.optNm} />
                        <p>
                          {option.optNm}
                  ...
                  : carAddCategoryOptions?.map((carAddCategoryOption, i) => (
                      <li key={i}>
                        ...
                        <ul className="cate-list-detail">
                          {carAddCategoryOption?.optionList?.map((option, idx) => (
                            <li className={classNames({ on: isExist(option.optCd) })} key={idx}>
                              <Tooltip simple={true} placement="bottomLeftLeft" width={560} exception="car-option">
                                <TooltipItem>
