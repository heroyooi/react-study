...
class PriceLineChart extends React.Component {
  ...
  componentDidMount() {
    ...
    const maxBaseValue = getNumberCutting(minMax.maxNum, 'C', 100);
  ...
  onDrawPoint(max, pixelToNumber) {
    this.props.data.values.forEach((value, index) => {
      ...
      onDragRoundRect(this.ctx, x - width / 2, y - this.state.topBaseOffset + 2, tooltipWidth, tooltipHeight + 4, 3, toolTipFillColor, tooltipStrokeColor);
      const text = getNumberWithCommas(value);
      const leftPadding = text.length === 3 ? 15 : 10;
      ...
      this.ctx.fillText(text, x - width / 2 + leftPadding, y - 18);
      this.ctx.fill();
      this.ctx.closePath();
    });
  }
  ...
  onDrawRecommandText(maxBaseValue, pixelToNumber, lines) {
    ...
    if (yHeight === 0 && lines.length === 2) {
      yCenter = lines[0].y + (lines[0].y - lines[1].y) / 2 - 5;
    }

    const left = this.props.hasMobile === true ? 5 : 14;
    this.ctx.beginPath();
    this.ctx.font = 'bold 16px Roboto, Noto Sans KR, AppleSDGothicNeo, Malgun Gothic, 맑은 고딕, 돋움, Arial, sans-serif';
    this.ctx.fillStyle = '#fff';
    this.ctx.fillText('매각권장', x1.x + left, yCenter - yHeight / 2);
    this.ctx.fillText('기간', x1.x + left + 10, yCenter - yHeight / 2 + 20);
  ...
  onDrawLastLine(maxBaseValue, pixelToNumber) {
    const recommandTo = this.props.data.values[4];
